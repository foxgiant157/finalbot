...
async function starts() {
	...
	client.on('message-new', async (mek) => {
		try {
			...
			const time = moment.tz('Brasil/Brasília').format('DD/MM HH:mm:ss')
			...
			switch(command) {
				...
				case 'sticker':
					...
					/*} else if ((isMedia || isQuotedImage) && colors.includes(args[0])) {
						...
						reply(`Envie fotos com legendas ${prefix}sticker ou a tag da imagem enviada`)
					}
					break
				case 'gtts':
					...
					rano = getRandom('.ogg')
					dtt.length > 600
					? reply('A maior parte do texto é bosta')
				...
				case 'meme':
					...
					buffer = await getBuffer(`https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcRRevwGP698_54WGcZOrtwI2S5abGZ1hCIPwQ&usqp=CAU`)
					client.sendMessage(from, buffer, image, {quoted: mek, caption: '.......'})
					break
				case 'memeindo':
					memein = await kagApi.memeindo()
					buffer = await getBuffer(`https://1.bp.blogspot.com/-iqCD9Kk_ZVQ/YALc2rdZE6I/AAAAAAAACAk/SG7YFGW4jjcZXmTX8uD65zUp-rGb586dACLcBGAsYHQ/s16000/IMG-20210116-WA0029.jpg`)
					client.sendMessage(from, buffer, image, {quoted: mek, caption: 'Fica tranquila'})
					break
				case 'porno':
					memein = await kagApi.memeindo()
					buffer = await getBuffer(`https://1.bp.blogspot.com/-YD-4s17zGjA/YALaAez-foI/AAAAAAAAB_4/8q-LncQXInYkg6G92YLFMK5rpBxub0MXgCLcBGAsYHQ/s16000/IMG-20210116-WA0027.jpg`)
					client.sendMessage(from, buffer, image, {quoted: mek, caption: 'Vai pra igreja com Hzin🙄'})
					break
				case 'dono':
					memein = await kagApi.memeindo()
				...
				case 'loli':
					memein = await kagApi.memeindo()
					buffer = await getBuffer(`https://1.bp.blogspot.com/-7Dl06RhdsKM/YALaIOaI7gI/AAAAAAAAB_8/0XNQtSZMBPIN7Pn9aBZUyIWvhDB6zxQ6QCLcBGAsYHQ/s16000/IMG-20210116-WA0028.jpg`)
					client.sendMessage(from, buffer, image, {quoted: mek, caption: 'hmm, então quer ver loli?'})
					break
				case 'hentai':
					memein = await kagApi.memeindo()
					buffer = await getBuffer(`https://1.bp.blogspot.com/-xcN4EhzQjIU/YALeegLnfVI/AAAAAAAACA4/0coLOI2jik84yfEee0Fi2CrxJzvrqfbwACLcBGAsYHQ/w466-h640/images%2B%25281%2529.jpeg`)
				...
				case 'setprefix':
					...
					prefix = args[0]
					reply(`Olá o prefixo do seu bot agora é: ${prefix}`)
					break
				case 'lol':
					loli.getSFWLoli(async (err, res) => {
						if (err) return reply('❌ *ERRO* ❌')
						buffer = await getBuffer(res.url)
						client.sendMessage(from, buffer, image, {quoted: mek, caption: 'Ingat! Outro Lolimu'})
					})
					break
				case 'upw':
				...
				case 'limpar':
					...
					reply('Agora os chats do whatsapp do seu bot esta limpo')
					break
				case 'teste':
					if (!isOwner) return reply('Você nao é dono do bot haha')
					if (args.length < 1) return reply('Ola meu dono ♕')
					anu = await client.chats.all()
        ...
        case 'promote':
					...
					if (mentioned.length > 1) {
						...
						mentions(`Esse cara aqui @${mentioned[0].split('@')[0]} acabou de virar adm, entao respeitem 😎👍`, mentioned, true)
				...
				case 'demote':
					...
					if (mentioned.length > 1) {
						...
						mentions(`Esse cara aqui @${mentioned[0].split('@')[0]} Foi rebaixado para membro comum 😂!`, mentioned, true)
				...
				case 'add':
					...
					if (!isGroupAdmins) return reply(mess.only.admin)
					if (!isBotGroupAdmins) return reply(mess.only.Badmin)
					if (args.length < 1) return reply('Bota o numero do cara ai man')
				...
				case 'kick':
					...
					if (mentioned.length > 1) {
						...
						mentions(`Esse carinha aqui levou ban com sucesso 🙊 @${mentioned[0].split('@')[0]}`, mentioned, true)
						client.groupRemove(from, mentioned)
					}
					break
				case 'listadmins':
					if (!isGroup) return reply(mess.only.group)
					teks = `Essa é a lista de admins do grupo *${groupMetadata.subject}*\nTotal : ${groupAdmins.length}\n\n`
					no = 0
				...
				case 'simih':
					...
					if (Number(args[0]) === 1) {
						if (isSimi) return reply('O modo Simi está ativado cuidado macacada 🙉')
						samih.push(from)
						fs.writeFileSync('./src/simi.json', JSON.stringify(samih))
						reply('Modo simi ativado com sucesso neste grupo hehe 🙊️')
				...
				case 'welcome':
					...
					if (args.length < 1) return reply('Hmmmm')
					if (Number(args[0]) === 1) {
						if (isWelkom) return reply('Voce ja atuvou isso KAKKAKA lerdão')
						welkom.push(from)
				...
				case 'clone':
					...
					try {
						...
						mentions(`Ok, Agora minha foto de perfil está igual a do @${id.split('@')[0]}`, [jid], true)
					} catch (e) {
						reply('Deu erro, aff')
					}
					break
